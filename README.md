...
│   │   │   ├── login_handler.go         # Authentication handler
│   │   │   ├── message_handler.go       # Message CRUD handler
│   │   │   ├── response.go              # HTTP response helpers
│   │   │   ├── user_handler.go          # User management handler
│   │   │   └── webhook_handler.go       # Webhook event handler
│   │   ├── http/
│   │   │   └── middleware.go            # HTTP middleware (auth, logging, etc.)
│   │   ├── repository/
│   │   │   ├── account.go               # Account DB repository
│   │   │   ├── db.go                    # Database connection setup
│   │   │   ├── firebase.go              # Firebase repository adapter
│   │   │   ├── message.go               # Message DB repository
│   │   │   ├── payment.go               # Payment DB repository
│   │   │   └── user.go                  # User DB repository
│   │   └── snowflake/
│   │       └── snowflake.go             # Snowflake ID generator adapter
│   ├── config/
│   │   └── config.go                    # App configuration loader
│   ├── core/
│   │   ├── domain/
│   │   │   ├── dto.go                   # Data transfer objects
│   │   │   ├── enum.go                  # Domain enumerations
│   │   │   └── model.go                 # Domain models
│   │   ├── ports/
│   │   │   ├── cache.go                 # Cache port interface
│   │   │   └── ports.go                 # All other port interfaces
│   │   └── services/
│   │       ├── account.go               # Account business logic
│   │       ├── auth.go                  # Authentication business logic
│   │       ├── customer.go              # Customer business logic
│   │       ├── firebase.go              # Firebase notification service
│   │       ├── message.go               # Message business logic
│   │       └── payment.go               # Payment business logic
│   ├── logger/
│   │   ├── log_setup.go                 # Logger initialization
...
# 🚀 Why We Use Bun Instead of GORM

This project was originally written with GORM but has since been refactored to use [Bun](https://bun.uptrace.dev/) (`github.com/uptrace/bun` v1.2.18) — a lightweight, fast SQL-first ORM for Go.

**Why Bun over GORM:**

- **SQL-first**: Bun lets you write idiomatic SQL while still providing a Go-friendly query builder. You stay in control of your queries.
- **Performance**: Bun is significantly faster than GORM — it avoids heavy reflection and generates leaner SQL.
- **PostgreSQL native**: With `bun/dialect/pgdialect` and `bun/driver/pgdriver`, Bun has first-class PostgreSQL support including `RETURNING`, `ON CONFLICT`, array types and more.
- **Simpler codebase**: Bun's API is smaller and more predictable than GORM's, reducing magic and unexpected behavior.
- **Migrations**: Bun ships with a built-in migration runner used in this project (`internal/migrations/`).

In general, the use of an ORM can simplify and speed up development, especially for CRUD operations. Bun strikes the right balance between raw SQL control and developer ergonomics.
